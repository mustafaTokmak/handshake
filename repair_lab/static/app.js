...
function renderComparison(){
 const cases=run?.suite?.cases||[false,true].flatMap(attack=>Object.keys(conditionNames).map(condition=>({attack,condition,status:'queued'})));
 const completed=cases.filter(c=>c.status==='complete').length;
 $('suite-status').textContent=run?.suite?`${completed}/6 finished · ${run.suite.status}`:'Six fresh runs, automatically';
